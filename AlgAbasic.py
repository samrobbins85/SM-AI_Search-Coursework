...
# Let's set up an infinity
infinity=float("inf")
# Start the tour at the first node
tour=[0]
tour_length=0
while len(tour)<len(distance_matrix[0]):
    first_node = distance_matrix[tour[-1]]
    # Now we can loop through, and for each index in the list, replace with infinity
    for i in tour:
        first_node[i]=infinity
    # And if we want to get the smallest value it's just
    minimum=min(first_node)
    # This will return the first occurrence of the number, which is fine for what we need
    minimum_index=first_node.index(minimum)
    tour.append(minimum_index)
    tour_length+=minimum
tour_length+=distance_matrix[tour[-1]][0]
